...
function dashboardUsernameFromPath(
  pathname: string,
  mountPath: string,
): string | null {
...
function parseScanLimit(value: string | null): number {
  ...
  if (!/^\d+$/.test(value)) {
    throw new DashboardRequestError(
      "scanLimit must be an integer from 8 to 60.",
    );
  }

  const scanLimit = Number(value);
  if (
    scanLimit < MIN_DASHBOARD_SCAN_LIMIT ||
    scanLimit > MAX_DASHBOARD_SCAN_LIMIT
  ) {
    throw new DashboardRequestError(
      "scanLimit must be an integer from 8 to 60.",
    );
