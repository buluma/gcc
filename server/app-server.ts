...
export function createHostedAppServer(options: HostedAppServerOptions) {
  ...
  if (oauthConfigured && !options.sessionSecret) {
    throw new Error(
      "Missing required environment variable SESSION_SECRET for OAuth sessions. See .env.example.",
    );
  }

  const sessionSecret =
    options.sessionSecret || getRandomBytes(32).toString("hex");
  ...
  if (!process.env.GITHUB_PUBLIC_TOKEN) {
    console.warn(
      "GITHUB_PUBLIC_TOKEN is not set; hosted public dashboards will use GitHub's anonymous REST quota.",
    );
  }
