...
async function createDashboardRequestError(
  response: Response,
): Promise<DashboardRequestError> {
...
function toDashboardErrorState(
  error: unknown,
...
function isGithubRateLimitRequestError(error: unknown): boolean {
  return (
    ...
    error.payload.code === "github_rate_limit"
  );
}
...
function cacheMatchesPublicViewer(
...
async function requestDashboard(
  ...
  const authMode =
    responseAuth === "oauth" || responseAuth === "public" ? responseAuth : null;
...
export default function DashboardPage({
  ...
  const dashboardSourceKey = demoMode
    ? "demo"
    : publicUsername
      ? `public:${publicUsername.toLowerCase()}`
      : "session";
  ...
  const [data, setData] = useState<DashboardPayload | null>(() =>
    demoMode ? createDemoDashboard() : null,
  );
  ...
  const dashboardApiPath = useCallback(
    (options: { force?: boolean; quick?: boolean } = {}) => {
      const base = publicUsername
        ? `/api/dashboard/${encodeURIComponent(publicUsername)}`
        : "/api/dashboard";
      ...
      try {
        ...
        if (!cancelled) {
          ...
          const cacheMatchesViewer =
            initialDashboardCache?.payload.viewer.login ===
            quickPayload.viewer.login;
...
function deriveDashboardView(
  ...
  const hiddenRepoNames = new Set(
    data.repos
      .filter((repo) => hiddenRepoIds.has(repo.id))
  ...
  const filteredRepos = scopedRepos
    ...
    .filter(
      (repo) =>
        filters.ciState === "all" || getRepoCiState(repo) === filters.ciState,
    )
        ...
        <div className="flex min-w-0 items-center gap-2.5">
          ...
          >
            ...
            <span className="text-lg font-semibold leading-none">
              {data?.viewer.login ?? "Command Center"}
            </span>
          </a>
        </div>
        <div className="flex min-w-0 flex-1 flex-col gap-2 md:flex-row md:items-center lg:max-w-[820px] xl:max-w-none">
          <div className="relative min-w-0 flex-1 md:min-w-48">
            ...
            {query ? (
              ...
              <kbd
                className="pointer-events-none absolute top-1/2 right-2 hidden -translate-y-1/2 rounded-sm border bg-muted px-1.5 font-mono text-[10px] leading-4 text-muted-foreground md:block"
                aria-hidden="true"
              >
                /
              </kbd>
          ...
          <div className="hidden min-w-0 items-center gap-2 px-1 text-sm text-muted-foreground md:flex lg:hidden xl:flex">
            <span className="truncate" aria-live="polite">
              {updatingDetails
                ? "Updating"
                : data
                  ? `Refreshed ${formatRelative(data.generatedAt)}`
                  : "Ready"}
            </span>
          </div>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                ...
                >
                  <Avatar className="size-8">
                    ...
                    <AvatarFallback>
                      {data?.viewer.login.slice(0, 2).toUpperCase() ?? "GH"}
                    </AvatarFallback>
                  </Avatar>
              ...
              <DropdownMenuContent align="end" className="w-48">
                <DropdownMenuLabel className="truncate">
                  {data?.viewer.login
                    ? `Signed in as ${data.viewer.login}`
                    : "Signed in"}
                </DropdownMenuLabel>
                <DropdownMenuSeparator />
  ...
  return (
    <div className="flex w-full shrink-0 flex-wrap items-center gap-2 text-sm md:w-auto md:flex-nowrap">
      <Select value={visibility} onValueChange={onVisibilityChange}>
        <SelectTrigger
          size="sm"
          className="h-8 min-w-32 flex-1 bg-card sm:flex-none md:w-32"
        >
      ...
      <Select value={language} onValueChange={onLanguageChange}>
        <SelectTrigger
          size="sm"
          className="h-8 min-w-36 flex-1 bg-card sm:flex-none md:w-36"
        >
      ...
      <Select value={ciState} onValueChange={onCiStateChange}>
        <SelectTrigger
          size="sm"
          className="h-8 min-w-24 flex-1 bg-card sm:flex-none md:w-24"
        >
