    ...
    <div className="min-h-screen bg-background text-foreground [text-wrap:pretty]">
      <div className="mx-auto max-w-[1320px] px-6 min-[520px]:px-10">
        <header className="flex items-center pt-9">
          <a
            href="/"
            className="flex items-center gap-[9px] text-[17px] leading-none font-semibold"
            aria-label="okgithub home"
          >
        ...
        <main>
          <section className="mx-auto max-w-[620px] pt-16 text-center">
            ...
            <p className="mt-4 text-[17px] leading-[1.6] font-normal text-muted-foreground">
              One view of everything: PRs, issues, commits, CI, and Actions
              billing across all your repos. No login needed unless you want to
              see the private stuff. Open source.
            </p>
            ...
            <div className="mx-auto mt-7 max-w-[460px]">
              <form
                ...
                onSubmit={(event) => {
                  if (
                    !normalizedUsername ||
                    !GITHUB_LOGIN_PATTERN.test(normalizedUsername)
                  ) {
          ...
          <section className="mt-[60px] grid grid-cols-1 gap-9 min-[860px]:grid-cols-3">
            <HomepageFeature title="Public view, no login">
              Any profile is a dashboard:{" "}
              <code className="rounded-[5px] bg-muted px-1.5 py-0.5 font-mono text-[12px] leading-none font-medium text-foreground">
                /username
              </code>{" "}
              shows public PRs, issues, commits, CI, and repos instantly.
            </HomepageFeature>
            <HomepageFeature
              title={
                PUBLIC_DEPLOYMENT
            ...
            <HomepageFeature title="Self-host or run local">
              ...
              CLI, or deploy to Cloudflare Workers or any Node host.
            </HomepageFeature>
          </section>
        </main>
...
function HomepageFeature({
  ...
  return (
    <div>
      <h3 className="mb-1.5 text-[15px] leading-[1.3] font-semibold">
        {title}
      </h3>
      <p className="text-[13.5px] leading-[1.55] font-normal text-muted-foreground">
        {children}
      </p>
    </div>
  );
